                            ...
                            <table class="table table-bordered table-hover">
                                ...
                                <template v-if="products.length > 0">
                                    <tr v-for="(product, index) in products" :key="index">
                                        <td>{{ index + 1 }}</td>
                                        <td>
                                            <img :src="ImageURL+product.image" width="40">
                                        </td>
                                        <td>{{ product.title }}</td>
                                        <td>${{ product.price }}</td>
                                        <td>
                                        ...
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenu21">
                                                    ...
                                                    <button @click="showProduct(product.id)" data-toggle="modal" data-target="#productView" class="dropdown-item" type="button">View</button>
                    ...
                    <div class="modal-body">
                        <table class="table table-borderless">
                            ...
                            <tr>
                                <th>Image</th>
                                <td>:</td>
                                <td><img :src="ImageURL+productData.image" class="img-thumbnail" width="50%"></td>
                            </tr>
                        </table>
                    </div>
...
<script>
    ...
    export default {
        data() {
            return {
                imgUrl: process.env.VUE_APP_API_URL_IMG,
                loader: false,
                message: '',
                products: [],
                productData: '',
        ...
        methods: {
            ...
            getProduct() {
                axios.get(this.Api.product,{ headers: this.$globalHelper.authHeader() })
            ...
            showProduct(product_id) {
                axios.get(this.Api.product+"/"+product_id, { headers: this.$globalHelper.authHeader() })
                    .then((response) => response.data)
                    .then((response) => {
                        if (response.status === true) {
            ...
            deleteProduct(product_id) {
                axios.delete(this.Api.product+"/"+product_id, { headers: this.$globalHelper.authHeader() })
                    .then((response) => response.data)
                    .then((response) => {
                        if (response.status === true) {
